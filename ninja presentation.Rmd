...
### Image from: https://www.cancer.gov/publications/dictionaries/cancer-terms/def/intrahepatic-bile-duct
...
data.f[Age %in% "All"][Sex %in% "Females"] %>% 
  ggplot(aes(Year,mrate, group=Country, colour = Country)) +
    ...
    ggtitle("For the Female population")+
    theme_dark() +
...
data.f[Age %in% "All"][Sex %in% "Males"] %>%  
  ...
  ylab("Mortality Rate") +
  scale_x_continuous(labels=data.f[,Year], breaks = data.f[,Year])+
  ggtitle("For the Male population")+
  theme_dark() +
  theme(plot.title = element_text(size=22, face = "bold", hjust = 0.5), 
        legend.position = "bottom",
        axis.title = element_text(size = 20),
...
```{r female population.ex3, echo=FALSE, fig.align="center", warning=FALSE}
library(kableExtra)
# For the female population
data.diff.F %>%
...
```{r Male population.ex3, echo=FALSE, fig.align="center", warning=FALSE}
# For the male population
...
- Podiamos fazer as duas em separado ou as duas juntas. O que acham? Apaguem o que não quiserem
