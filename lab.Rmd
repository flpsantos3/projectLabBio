...
### Which country changed (increase or decreased) the most in percentage terms between 2011 and 2016? - Plot and tabulate the percentage change overtime by country.

```{r}
data.diff <- data.f[Year != 2016][Year %in% 2011]
data.diff[,per := ((data.f[Year %in% 2016]$mrate - data.diff$mrate)/data.f[Year %in% 2011]$mrate*100)]
```

```{r}
data.all <- data.f[Age %in% "All"]

data.diff <- data.all[Year != 2016][Year %in% 2011]
data.diff[,per := ((data.all[Year %in% 2016]$mrate - data.diff$mrate)/data.all[Year %in% 2011]$mrate*100)]

pct.diff <- data.diff[!is.na(Country)][
  order(Country, Year, Sex, per), head(.SD, 1), by = .(Country, Year, Sex)][
    , .(Country, Year, Sex, per)]

setnames(pct.diff, "per", "Difference in % between 2016 and 2011")
pct.diff
```
